...
grep -q env manifest.jenkins.yml &&
  echo "    LANDSAT_HOST: $landsat_host\n    SENTINEL_HOST: $sentinel_host\n    BF_TIDE_PREDICTION_URL: $tide_prediction_url" >> manifest.jenkins.yml ||
  echo "  env: {LANDSAT_HOST: \"$landsat_host\", SENTINEL_HOST: \"$sentinel_host\", BF_TIDE_PREDICTION_URL: \"$tide_prediction_url\"}" >> manifest.jenkins.yml
